...
def test_data_science_pipeline(caplog, dummy_data, dummy_parameters):
    pipeline = (
        create_ds_pipeline()
        .from_nodes('candidate_modelling_pipeline.split_data_node', 'active_modelling_pipeline.split_data_node')
        .to_nodes('candidate_modelling_pipeline.evaluate_model_node', 'active_modelling_pipeline.evaluate_model_node')
    )
    catalog = DataCatalog()
    catalog.add_feed_dict(
        {
            "model_input_table" : dummy_data,
            "params:candidate_modelling_pipeline.model_options": dummy_parameters["model_options"],
